    ...
    QObject(parent), QGraphicsItem()
{
    ...
    steps = 0;      // Задаём стартовое положение ножек паука
...
{
    // Определяем расстояние до божьей коровки
    QLineF lineToTarget(QPointF(0, 0), mapFromItem(target, 0, 0));
    // Угол поворота в направлении к божьей коровке
    qreal angleToTarget = ::acos(lineToTarget.dx() / lineToTarget.length());
    if (lineToTarget.dy() < 0)
        angleToTarget = TwoPi - angleToTarget;
    angleToTarget = normalizeAngle((Pi - angleToTarget) + Pi / 2);

    /* В Зависимости от того, слева или справа находится божья коровка от Паука,
    ...
    // Бежим в сторону божьей коровки
